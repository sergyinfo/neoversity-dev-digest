...
1. **`e2e/specs/10-evals.flow.json` HAS now been run** against the hermetic stack (`./scripts/e2e.sh`), and it found two real defects in the flow itself — both assertions written from the source rather than from the browser. (a) Five assertions compared i18n source strings against text that `SectionLabel` and the table headers uppercase via CSS; `innerText` honours `text-transform`, `textContent` does not. (b) One assertion expected a whole sentence from the degraded prompt to appear contiguously in the prompt diff — but the diff is word-level, so LCS aligns the words the two prompts share and interleaves the rest, leaving no phrase intact in the DOM. Both fixed. **Every assertion this feature owns now passes live**, including AC-18/AC-19's Compare gate at 0, 1 and 2 selections and a real prompt diff in the modal.

   The flow still ends red at the shared `open the PR row` step, which also fails flows 04, 05, 08 and 09 on this machine. That is the environmental flakiness recorded in `e2e/INSIGHTS.md` (2026-08-28): `find text` does not poll the way `wait` does, and opening a PR refreshes from GitHub. Evidence it is intermittent rather than a break: flow 02 uses the same step and **passed** in the same run. CI has no PAT configured, so it does not surface there.
...
7. **No e2e baseline was captured at Stage 0.** typecheck, unit and integration were; the browser flows were not. When five flows came back red, I could not tell ours from pre-existing without diagnosing each one. The protocol asks for that baseline precisely to avoid it — the same shape of omission this run kept finding in other people's checks.
8. **My own process errors, recorded because they cost real time:** I swept track T4's in-progress files into a commit labelled as three other tracks, and had to rebuild it; a path glob (`evals-*.test.ts`) silently missed the 445-line integration test; I folded S14's INSIGHTS half onto myself and then did not do it — `plan-verifier` caught that from the diff, not from my report; and I patched a JSON flow by string substitution with an unescaped quote, then ran the stack anyway because the edit and the run were separated by a newline instead of `&&`, so a full run executed against the unmodified file. Two of the three are the same mistake: operating on structured data with a text pattern, and letting a check's result change nothing.
