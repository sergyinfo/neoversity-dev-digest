...
> All six blocking questions are answered and all eight recommendations accepted — see **Settled premises**. `plan-verifier` should check the work against those premises rather than re-deriving them.
>
...
These are decided. Do not re-open them mid-implementation; if one turns out to be wrong, stop and escalate rather than improvising.

**Blocking questions — all answered (a):**

| BQ | Decision | Binding consequence |
|---|---|---|
| BQ-1 | **Extend `server/src/db/seed.ts`** — patch hunks for the three patch-less `pr_files`, `acceptedAt` on ~5 findings, `dismissedAt` on ~4 (2–3 deliberate style nits), seeded review attributed to the Security Reviewer agent | S12 exists; `verify:l06` asserts "≥ 8 cases" offline |
| BQ-2 | Owner = `review.agent_id` when set; else `agent_id` from the request body; else **422 naming the reason** | S1 gains an optional body; S4 implements the three-way resolution; S12 makes the first branch reachable on demo data |
| BQ-3 | `input_diff` = the **single file's slice**, via `sliceDiff` from `@devdigest/reviewer-core` | S4; keeps a run ≈ N small calls |
| BQ-4 | **The batch is the unit.** `GET /agents/:id/eval-runs` returns batches; Compare takes two `batch_id`s; `traces_passed`/`traces_total` carry batch-level counts | S3 batch aggregation; S5 adds a route **not in the spec's table** — flagged in Risks |
| BQ-5 | **Synchronous**, exactly as the approved contract states, with a 50-case cap and `POST /pulls/:id/review`'s rate limit | S4, S5; no SSE, no job runner, no run-status surface |
| BQ-6 | Score the **post-grounding** set (`outcome.review.findings`); `citation_accuracy` reports what grounding removed | S2, S4 |

**Recommendations — all eight accepted:**

| REC | Decision | Where it lands |
|---|---|---|
| REC-1 | `agent: { id, name, … }` in the `ActualOutput` envelope | S1, S4, S10 |
| REC-2 | Render **"n/a"**, not "100%", when `TP + FP = 0` | S9, S10 (spec open question 2) |
| REC-3 | Reuse `sliceDiff` rather than hand-slicing | S4 |
| REC-4 | Static, model-free "no LLM in scoring" check with the comment-stripper | S13 |
| REC-5 | 50-case cap + `{ rateLimit: { max: 10, timeWindow: '1 minute' } }` | S4, S5 |
| REC-6 | **Per-skill content hash in the snapshot** (spec open question 3) | S1 (envelope), S4 (run loop), S3 (query) |
| REC-7 | **`e2e/specs/10-evals.flow.json`** — creation-affordance + dashboard read only, never a run | **S15**, plus a second half of S12 |
| REC-8 | Append the four session traps to `server/INSIGHTS.md` and `client/INSIGHTS.md` | S14 |

**Where REC-6 and REC-7 add cost, plainly:**

- **REC-6** was rated *idea*. It costs: one `createHash('sha256')` per linked skill per case (negligible runtime), a widened envelope, one extra repository query, and one test. **It also surfaces a trap:** the `skills` table has **no `slug` column** — `server/src/db/schema/skills.ts:5-21` has `id, workspaceId, name, description, type, source, body, enabled, version`. The only "slug" in the codebase is a doc-comment about the CI manifest (`contracts/eval-ci.ts:148-150`). The snapshot therefore stores `{ id, name, version, content_hash }` per skill, and `skills.version` (already bumped per edit) is a second, free staleness signal. Anyone implementing "skill slugs" literally will invent a column that does not exist.
- **REC-7** was rated *idea* **and scope-enlarging**, and it is the more expensive of the two. It adds a fifth package to the verification surface, requires a **running stack and a freshly-seeded DB** (`./scripts/e2e.sh` hermetic runner), and adds a browser-runner dependency to the gate. **It also forces an addition to S12:** every existing flow is read-only against seeded data (`e2e/README.md:35-36`, and flow `08`'s own note "no POST is made anywhere in this flow"), so the flow **cannot click "Turn into eval case"** — clicking mutates and breaks the freshly-seeded precondition. To assert a populated dashboard and a working Compare gate, **S12 must additionally seed ≥ 8 `eval_cases` rows and two synthetic `eval_runs` batches** (plain rows, no model). This is *additive*, not a substitute for the one-click flow — that stays covered by S6's integration test and the manual experiment. If you want to back REC-7 out, delete S15 and the second half of S12; nothing else changes.
