...
```sh
# a) delete the conventions skill so "Create skill" does not hit a name collision
curl -s http://localhost:3001/skills \
  | python3 -c "import json,sys;print([s['id'] for s in json.load(sys.stdin) if s['name'].endswith('-conventions')][0])" \
  | xargs -I{} curl -s -X DELETE http://localhost:3001/skills/{}

# b) unlink the four skills from API Contract Reviewer — run A must start clean
curl -s -X POST http://localhost:3001/agents/<AGENT_ID>/skills \
  -H 'content-type: application/json' -d '{"skill_ids":[]}'

# c) delete the two experiment runs on PR #3 so the Agent-runs tab starts empty
```

Conventions themselves do **not** need clearing — a re-scan replaces them, and
running the scan on camera is the point.

Keep `<AGENT_ID>` and the four skill ids in a scratch file; you will need them for
the relink in step 6.
...
1. Pick the **small-cell suppression** card (`sponsor-facet.ts`).
...
1. Note the counter: **`4 of 5 accepted`**.
...
- Do not run `Run Review ▾ → all agents` — it would fan out to every enabled agent and cost real money.
